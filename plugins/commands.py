...
async def start(client, message):
    if message.chat.type in ['group', 'supergroup']:
        buttons = [
            ...
            [   InlineKeyboardButton('➕ 𝙰𝙳𝙳 〽𝙴 𝚃𝙾 𝚈𝙾𝚄𝚁 𝙶𝚁𝙾𝚄𝙿 ➕', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
            ]
            ]
...
async def telegraph(client, message):
    ...
    try:
        ...
        await message.reply(
            ...
            [
                [
                    InlineKeyboardButton(text="𝙾𝙿𝙴𝙽 𝙻𝙸𝙽𝙺", url=f"https://telegra.ph{response[0]}"),
                    InlineKeyboardButton(text="𝚂𝙷𝙰𝚁𝙴 𝙻𝙸𝙽𝙺", url=f"https://telegram.me/share/url?url=https://telegra.ph{response[0]}")
             ], [
                InlineKeyboardButton(text="⏪ 𝙱𝙰𝙲𝙺", callback_data="close_data")
           ]]
        )
    )
    finally:
